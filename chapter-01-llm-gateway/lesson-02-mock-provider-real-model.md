...
> 上一课：[第 1 课：建立一个可测试的后端](./第01课-建立一个可测试的后端.md)  
> 下一课：[第 3 课：接入 OpenAI-compatible 模型服务](./第03课-接入 OpenAI-compatible 模型服务.md)

### 你将完成什么

定义统一的模型 Provider 接口，并实现一个确定性的 Mock Provider。你将在不依赖外部 API 的情况下完成大多数后端和前端练习。

### 为什么不能一开始就绑死某个模型

模型服务会限流、涨价、升级接口或暂时不可用。业务代码若到处直接调用某家 SDK，后面切换模型会非常痛苦。Provider 抽象的目的不是“设计很优雅”，而是把供应商差异隔离在一处。
