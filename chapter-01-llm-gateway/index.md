...
这几年，搭一个能稳定调用大模型的后端，成了许多人的刚需。环境、路由、Provider、前端流式展示——环节一多，**第一次调用失败时，往往分不清问题出在哪一层**。

本章就是来解决这个基础问题的：先把「稳定调用模型」做正确。不做 Agent Loop，也不做工具调用。

读完并跟着做完七课，你将得到一个可运行的前后端项目。具体来说：
...
建议按目录顺序学习：先搭可测试的后端，再接 Mock，最后再换真实模型。
