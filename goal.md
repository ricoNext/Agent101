# AI Agent 全栈工程师训练营课程大纲

> 版本：v3.0
>
> 设计基线：在 `goal_v0.md` 的工程主线基础上重新规划。
>
> 课程结构：七章核心课 + 七个项目里程碑 + 进阶专题 + 二选一综合项目。

---

## 一、课程定位

本课程面向希望转型为 AI Agent 开发工程师、大模型应用开发工程师、
RAG / Agent 工程师的传统后端、全栈或平台工程师。

课程不以低代码平台操作、模型训练或行业概念综述为主，
而是围绕一个持续演进的 **Agent Engineering Platform**，训练学员完成：
...
课程最终目标不是“会调用模型”或“能搭一个 Demo”，
而是让学员具备独立构建、评测、部署和持续迭代 Agent 系统的工程能力。

### 目标学员

- 具备后端、全栈、平台工程或测试开发经验，希望转向 AI Agent 工程方向
- 已经能调用大模型 API，但缺少完整 Agent 工程方法
- 做过 RAG、Workflow 或 Agent Demo，希望补齐生产化能力
- 希望形成可用于求职、晋升或内部落地的工程作品集
...
- LLM 应用工程
- Tool Runtime 与 MCP
- Agent Loop、State、Sandbox 与 Harness
- Context、Memory 与 RAG
- Multi-Agent、Skill 与 Eval
- 可观测性、成本、安全和生产部署
...
所有章节围绕同一个 Agent Engineering Platform 演进，
不为每章重新创建互不相关的 Demo。

平台从 LLM Gateway 起步，逐步加入 Tool Runtime、Agent Loop、Context Manager、
Codebase RAG、Multi-Agent、Eval、Trace、Replay 和生产治理能力。

### 2. 三层内容分级

课程内容按重要程度划分为三层：

- **核心必修**：Agent 工程师必须掌握，进入课堂实现和项目验收
- **进阶实战**：用于扩大工程视野，可安排为工作坊或挑战作业
- **选修专题**：变化快或依赖具体业务，不占用核心课时

具体划分：

| 层级 | 内容 |
| --- | --- |
| 核心必修 | LLM Gateway、Tool Runtime、MCP、Agent Loop、State、Sandbox、Context、Memory、RAG、Skill、Eval、Trace、成本治理、生产部署 |
| 进阶实战 | A2A 跨框架协作、AI Coding Agent 实战、复杂 Multi-Agent、在线 Eval、私有化推理 |
| 选修专题 | ACP、AG-UI / A2UI、Governance Agent、Kubernetes、Firecracker / E2B、行业法规详解 |

### 3. 原理优先，产品作为案例

课程先讲稳定的工程原理，再用具体产品辅助理解。

- LangGraph 用于演示状态图和中断恢复，不将课程绑定到单一框架
- Claude Code、Cursor 等用于 AI Coding Agent 工作坊，不替代 Codebase Agent 自建过程
- A2A 用于理解跨 Agent 互操作，不要求所有学员实现完整协议栈
- DeerFlow 等开源项目用于架构阅读和综合案例，不作为唯一项目模板

### 4. Eval 贯穿全程

Eval 虽然独立成章，但评测不是到第六章才开始。
...
- Tool Runtime 阶段处理参数、权限、确认和审计
- Sandbox 阶段处理文件、命令、网络和资源边界
- RAG 阶段处理知识权限、来源和敏感数据
- Multi-Agent 阶段处理委托权限和预算
- 生产阶段统一处理身份、租户、密钥、日志和合规检查
...
1. 设计统一模型调用层，处理多模型、流式输出、结构化输出、重试和成本记录。
2. 设计带 Schema、权限、风险分级、审计和失败恢复的 Tool Runtime。
3. 接入或实现 MCP Server，并解释 Function Calling 与 MCP 的职责边界。
4. 实现 observe -> plan -> act -> observe -> finalize 的 Agent Loop。
5. 使用 State Machine、Checkpoint 和 Replay 支撑长程任务恢复与问题复现。
6. 为文件、Shell、网络和高风险写操作建立 Sandbox 与 Human-in-the-loop 边界。
7. 设计 Context Budget、Working Memory、Project Memory 和上下文压缩策略。
8. 构建带 Hybrid Search、Rerank、Citation 和权限过滤的 Codebase RAG。
9. 判断何时使用单 Agent、固定 Workflow 或 Multi-Agent，并设计可靠的任务委托协议。
10. 建立 Golden Dataset、自动评测、LLM-as-Judge、回归测试和失败分析流程。
11. 通过 Trace、Metrics、版本管理、灰度和回滚治理 Agent 的生产运行。
12. 用任务成功率、延迟、成本、人工接管率和失败恢复率证明项目价值。

---

## 四、课程全景与建议课时

建议核心课程为 **96 课时**。项目开发贯穿课堂，不额外叠加同等体量的新项目。
实际授课可按下表比例缩放。

| 章节 | 主题 | 建议课时 | 项目里程碑 |
| --- | --- | ---: | --- |
| 第一章 | LLM API、Prompt、Structured Output 与 Gateway | 10 | M1：LLM Gateway |
| 第二章 | Function Calling、Tool Runtime 与 MCP | 14 | M2：Agent Tool Runtime |
| 第三章 | Agent Loop、State、Harness 与 Codebase Agent | 18 | M3：Codebase Agent |
| 第四章 | Context Engineering、Memory 与 Codebase RAG | 14 | M4：Context & RAG |
| 第五章 | Multi-Agent、Subagent、Skill 与 A2A | 12 | M5：Agent Collaboration |
| 第六章 | Agent Eval 工程 | 12 | M6：Agent Eval Platform |
| 第七章 | 生产工程、FinOps、安全治理与部署 | 12 | M7：Production Ready |
| 综合复盘 | 架构评审、作品集整理与答辩 | 4 | 最终项目答辩 |

进阶和选修专题建议另设 12-20 课时，不占用上述核心课时。
...
完成从传统 HTTP API 调用到可治理 LLM 服务的迁移。
本章只解决“如何稳定、统一、可观测地调用模型”，暂不引入 Agent Loop。

### 1. AI Agent 工程师能力模型

- 后端工程能力如何迁移到 Agent 工程
...
Chain-of-Thought、Self-Consistency 等方法以理解适用边界为主，
不把“提示词技巧堆叠”作为课程重点。
...
WebSocket 作为对比内容，不要求所有项目同时实现两种协议。
...
让模型从“生成内容”升级为“调用受控工具完成任务”，
构建包含注册、发现、校验、权限、审计、超时和失败恢复的工具运行时。
...
本章不以“Agent 数量多”为目标，而以复杂任务是否因此更可靠为判断标准。
...
建立可量化、可复现、可回归的 Agent 质量体系，
让 Prompt、Model、Tool、RAG 和 Workflow 的改动都有数据依据。
...
- Tool Runtime 与 MCP
- Agent Loop、State Machine 与 Checkpoint
- Sandbox 与 Human-in-the-loop
- Context Manager 与 Project Memory
- Codebase RAG
- Multi-Agent 与 Skill System
- Agent Eval Platform
- Trace、Replay 与 Metrics
- FinOps、版本、灰度和回滚
- 权限、审计、租户隔离和生产部署
...
- 事实核验、引用追溯和可信度评估
- 研究 Skill 沉淀
- 报告生成、Eval、Trace 和成本统计

#### 方向 B：软件工程 Agent / 软件工厂

- 需求分析、任务拆解和代码修改
- 多角色 Agent 的职责和权限边界
- Git、代码审查、测试和 CI 集成
- Worktree 或 Sandbox 隔离
- 质量门禁、人工审批和失败恢复

两个方向共用同一平台能力，不要求学员重复建设底层基础设施。

### 3. 四类作品成果

| 编号 | 作品成果 | 证明的核心能力 |
| --- | --- | --- |
| 1 | Agent Foundation Platform | LLM Gateway、Tool Runtime、MCP、权限与审计 |
| 2 | Codebase Agent | Agent Loop、State、Sandbox、Context、Memory 与 RAG |
| 3 | Agent Eval & Operations Console | Benchmark、Trace、Replay、版本、成本与生产治理 |
| 4 | 二选一综合业务项目 | 将平台能力转化为真实业务交付的能力 |
...
1. 原理与边界：解释为什么需要该能力，以及何时不应该使用。
2. 最小实现：先完成可运行的最小闭环。
3. 工程增强：加入校验、权限、恢复、Trace 和成本等能力。
4. 失败演练：主动制造超时、错误、越权和上下文缺失。
5. Eval 对比：用统一任务集验证改动是否有效。
6. 里程碑评审：检查代码、架构、运行证据和未解决风险。
...
- **可运行**：有明确启动方式和最小演示任务
- **可验证**：有验收数据和失败样本，不只展示成功截图
- **可追踪**：关键模型、工具和 Agent 步骤具有 Trace
- **可恢复**：长任务能取消、失败或从 Checkpoint 恢复
- **有边界**：权限、预算、超时和高风险动作受到控制
- **可说明**：能够解释设计选择、替代方案和已知限制

---

## 八、进阶与选修专题

这些专题不影响核心课程毕业，可根据班级背景和行业变化独立更新。
...
- ACP：编辑器与 Coding Agent 互操作
- AG-UI 与 A2UI：Agent 和前端交互协议对比
- Firecracker / E2B 等远程 Sandbox
- Kubernetes 部署与弹性伸缩
- Governance Agent 原型
- EU、中国、美国和特定行业的 AI 法规概览

选修内容以稳定原理为中心，具体产品、协议版本和市场数据不写入长期核心能力要求。

---

## 九、课程最终目标

学员完成课程后，应能独立回答并用代码证明以下问题：
