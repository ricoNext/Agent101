# 第 4 课：建立 Tool Runtime 执行链

> 章节导航：[第二章：Function Calling、Tool Runtime 与 MCP](./index.md)  
> 上一课：[第 3 课：实现 Tool Registry](./lesson-03-tool-registry.md)<br>
> 下一课：[第 5 课：实现超时、取消与有限重试](./lesson-05-tool-timeout-retry.md)

## 一、你将完成什么

第 3 课的 Registry 能回答“这个调用名当前应该解析到哪个版本”，但它仍然不执行任何业务代码。模型返回的 `ToolCall` 只是一个不可信的调用提案：名称可能不存在，参数可能不符合 Schema，模型也不能自行声明用户身份或权限。
...
本课只实现一次调用的正确边界。超时、取消、有限重试、幂等和并发调度会在第 5–6 课实现；完整的资源级授权在第 7–8 课实现；确认请求的持久化与审批审计在第 9–10 课实现。
...
下面的实现假设第 2 课已有 `validate_input()`、`validate_output()` 和 `ToolContractError`，第 3 课已有 `ToolRegistry`、`ToolDiscoveryContext` 与 `ToolNotAvailable`。
...
这里的 `order_store.find_for_tenant()` 是资源级授权的一个示例。第 7–8 课会把这种检查抽成更系统的授权策略，但本课已经规定：实现只能从可信上下文获得身份，不能相信模型自己附带的 `actor_id`。
...
确认凭证必须绑定调用 ID、工具版本、用户和失效时间。只传一个布尔值 `confirmed=True` 很容易被错误复用；第 9–10 课会实现持久化审批记录和一次性消费。
...
参数和结果不要默认原样写日志。可以保存 Schema 校验后的摘要、字段数量或经过脱敏的业务标识；高敏感工具还应记录访问策略的版本和确认决定。第 14 课会把这些事件接到 CLI 和 Tool Call Trace 页面。
...
网络超时不等于服务端没有完成写操作。重试逻辑需要幂等键、退避、取消和最大尝试次数，留给第 5–6 课的执行器实现。
...
建议答案方向：第 1 项属于资源级授权拒绝，具体错误码由第 7–8 课统一；第 2 项是业务约束错误，不是 JSON Schema 错误；第 3 项是 `confirmation_required`；第 4 项是 `invalid_tool_output`；第 5 项是统一的 `tool_not_available`，不应泄露目录是否曾经存在该工具。
