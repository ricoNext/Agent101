...
七章核心课程按能力依赖顺序学习，每章交付一个可验收的项目里程碑，共同演进为同一套 Agent Engineering Platform。

## 更新进度

状态说明：`已完成` 表示课次讲义已可阅读；`进行中` 表示目录已定、部分课次已写；`仅大纲` 表示只有章节总览，课次正文待写。

| 章节 | 主题 | 项目里程碑 | 讲义状态 | 进度说明 |
| --- | --- | --- | --- | --- |
...
| [第七章](./chapter-07-production-governance/) | 生产工程、FinOps、安全治理与部署 | M7：Production Ready | 仅大纲 | 章节目标与模块已定，课次待设计 |

当前主线写作重点：按课程目录逐节补全第三至第五章讲义内容。

课次在各章内独立编号，每章都从第 1 课开始。调整某章的课数时，不需要修改后续章节的课号。

建议从第一章开始按序推进；Eval、安全与治理从第一章起逐步加入平台，而不是最后才补。
