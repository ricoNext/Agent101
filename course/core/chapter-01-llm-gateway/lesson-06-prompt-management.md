# 第 6 课：管理 Prompt 模板与版本

> 所属章节：[第一章：LLM API、Prompt、Structured Output 与 Gateway](./index.md)  
> 上一课：[第 5 课：接入 OpenAI-compatible 模型服务](./lesson-05-openai-compatible-provider.md)  
> 下一课：[第 7 课：定义 Structured Output 契约](./lesson-07-structured-output-contract.md)
...
- 说明 Chain-of-Thought 与 Self-Consistency 的适用边界
...
[Chain-of-Thought](https://www.promptingguide.ai/techniques/cot)（CoT）和 [Self-Consistency](https://www.promptingguide.ai/techniques/consistency) 只要求理解适用边界，不要当成默认技巧往每个 Prompt 里堆。

CoT 让模型把推理步骤写出来，适合确实需要分步计算或排查的问题，但会增加输出 Token 和延迟。分类、抽取、已有 JSON Schema 的任务通常不需要“先思考再回答”，结构应由 Schema 约束。Self-Consistency 是对同一 Prompt 多次采样再投票，稳定性可能提高，费用和延迟会按采样次数上升。第一章的摘要任务和 Golden Tasks 不使用这种方法。需要可验证的中间结果时，优先用 Structured Output 或固定 Workflow。
...
- 能说明 CoT 与 Self-Consistency 何时不该使用
