# 第 11 课：实现可取消的 SSE 流式接口

> 所属章节：[第一章：LLM API、Prompt、Structured Output 与 Gateway](./index.md)  
> 上一课：[第 10 课：建立调用观测与成本治理](./lesson-10-observability-cost.md)  
> 下一课：[第 12 课：创建 Gateway 前端控制台](./lesson-12-gateway-console.md)  
...
所谓 **SSE（Server-Sent Events）**，字面上讲，就是服务器持续往客户端推事件。更准确的说法是：你先定义一套自己的事件协议，再把 Provider 的字符流翻译成这套协议。前端只认识你的事件，不直接认识任何 Provider 的原始 SSE 格式。

本章用 SSE 实现流式输出，[WebSocket](https://developer.mozilla.org/zh-CN/docs/Web/API/WebSockets_API) 只作对比，不要求同时实现。SSE 是服务端到客户端的单向推送，正好匹配“模型持续生成、客户端逐块消费”。WebSocket 是双向通道，更适合客户端要在同一连接上持续发指令、或做复杂会话同步的场景；它在断线重连和心跳上通常更完整，协议与运维成本也更高。Gateway 先把单向事件协议做稳定。以后如果需要双向控制面，再单独增加 WebSocket，而不是让前端同时解析两套模型事件。
...
流式事件需要覆盖 token、status、usage、error、done 五类语义。Gateway 用自己的事件名承载这些语义，不照抄厂商字段：

| 目标语义 | Gateway 事件 | 放在哪里 |
| --- | --- | --- |
| status | `event` 字段 | `run.started`、`message.delta`、`run.completed`、`run.failed` |
| token | `message.delta` | `data.text` |
| usage | `run.completed` | `data.usage`；无真实 Token 时仍保留字段，值为 `null` |
| error | `run.failed` | `data.code`、`data.message` |
| done | 终态事件 | 正常为 `run.completed`，失败为 `run.failed` |
...
结束事件记录本次流式调用的耗时、输出字符数和 `usage`。当前最小 Provider 协议只产出文本，没有可靠的流式 Token 统计，所以 `usage` 必须显式写成 `null`，不能省略字段，也不能用 `output_characters` 冒充 Token。以后 Provider 若在流结束时返回真实 usage，再填入 `input_tokens` 与 `output_tokens`，调用记录和基线报告才能与普通调用使用同一套口径。
...
- `run.completed` 包含 `latency_ms`、`output_characters` 和 `usage`
- 无真实 Token 时 `usage` 为 `null`，不使用字符数冒充
- 失败也会发送一个可处理事件
- 能说明第一章为什么用 SSE，而把 WebSocket 留作对比
- 客户端取消后，异步生成器和上游连接会关闭
- 取消不会被包装为普通 Provider 失败或触发 fallback

## 八、小结

下一课会创建 Gateway 前端控制台，用图形界面调试 Streaming、模型路由、Run ID 和停止操作，并验证本课的流式取消与错误终态。
