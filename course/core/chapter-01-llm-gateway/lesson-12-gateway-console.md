# 第 12 课：创建 Gateway 前端控制台

> 所属章节：[第一章：LLM API、Prompt、Structured Output 与 Gateway](./index.md)  
> 上一课：[第 11 课：实现可取消的 SSE 流式接口](./lesson-11-sse-streaming.md)  
> 下一课：[第 13 课：构建 Gateway 验收任务集](./lesson-13-gateway-acceptance-tasks.md)  
> [参考代码基线](https://github.com/ricoNext/agent-platform/tree/chapter-06)

## 一、你将完成什么

创建 Next.js Gateway 控制台，调用刚刚完成的 SSE 接口，逐步渲染模型输出，并提供停止按钮。这个页面是后续 Prompt、模型路由和调用观测界面的起点。
...
export default function Home() {
  ...
  const [route, setRoute] = useState("dev");
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/v1/chat/stream`, {
        ...
        body: JSON.stringify({ messages: nextMessages, model: route }),
      ...
      while (true) {
        ...
        for (const rawEvent of events) {
          ...
          if (agentEvent.event === "run.completed") {
            ...
            setLatencyMs(agentEvent.data.latency_ms ?? null);
            setStatus("已完成");
          }
  ...
  return (
    <main style={{ maxWidth: 720, margin: "40px auto", fontFamily: "sans-serif" }}>
      ...
      {latencyMs !== null && <p>耗时：{latencyMs} ms</p>}
      ...
      <form onSubmit={onSubmit}>
        <label htmlFor="route">模型路由</label>
        <select
          id="route"
          value={route}
...
- “停止”不仅要改变页面状态，还要依赖第八课的取消传播关闭 Provider 流；
